...
const generateImage = async()=>{
  try {
   ...
   // handleImage(blob, `thumbnail-${Date.now()}.png`)

   handleImage(blob, `thumbnail-${uuidv4()}.png`)

  } catch(error){
   console.log(error)
   toast({title:'Erorr generating thumbnail', variant :'destructive'})
  }
  ...
  return (
    ...
    {isAiThumbnail ? (
        ...
        <div className="flex flex-col items-center gap-1">
         <h2 className="text-12 font-bold text-white-3">
          Click to upload
          </h2>
          <p className="text-12 font-normal text-white-1">SVG, PNG, JPG, or GIF (max. 1080x1080px)</p> 
        </div>
      </div>
        )}
            {image && (
